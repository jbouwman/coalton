...
(defmethod node->code ((node env:name-entry))
  (list :name-entry
        (env:name-entry-name node)
        (env:name-entry-type node)
        (env:name-entry-docstring node)
        (env:name-entry-location node)))

(defun code->name-entry (name type docstring location)
  (env:make-name-entry :name name
                       :type type
                       :docstring docstring
                       :location location))

(setf (gethash :name-entry *cc*) #'code->name-entry)
  ...
  (case name
    ((coalton-impl/typechecker/environment::add-specialization
     ...
     (destructuring-bind (pred) args
       `(setf env (,name env ,(source-form pred)))))
    ...
    ((coalton-impl/typechecker/environment::set-function-source-parameter-names)
     (destructuring-bind (env-name entry) args
       `(setf env (,name env ',env-name (list ,@(mapcar #'source-form entry))))))
    (t
     (destructuring-bind (env-name entry) args
       `(setf env (,name env ',env-name ,(source-form entry)))))))
