...
(defun emit-forms (stream &rest forms)
...
(defun compile-translation-unit (stream translation-unit monomorphize-table env)
        ...
        (loop :for scc :in sccs
              :for bindings
                := (remove-if-not (lambda (binding)
                                  ...
                                  definitions)
              :do (compile-scc stream bindings env))
...
(defun compile-scc (stream bindings env)
  (declare (type binding-list bindings)
           (type tc:environment env))

  ;; Predeclare symbol macros
  (loop :for (name . node) :in bindings
        :do (emit-forms stream `(global-lexical:define-global-lexical ,name ,(tc:lisp-type (node-type node) env))))

  ;; Compile functions
  (loop :for (name . node) :in bindings
        :when (node-abstraction-p node)
          :do (progn
                (compile-function stream name node env)
                (emit-forms stream
                            `(setf
                              ,name
  ...
  (loop :for (name . node) :in bindings
        ...
        :when (and docstring (not settings:*coalton-skip-update*))
          :do (emit-forms stream `(setf (documentation ',name 'variable)
                                        ,docstring))
        :when (and entry (node-abstraction-p node) (not settings:*coalton-skip-update*))
          :do (emit-forms stream `(setf (documentation ',name 'function)
                                        ,docstring))))

(defun compile-function (stream name node env)
