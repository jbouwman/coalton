...
(defun parse-package-clause (toplevel-package form file)
  ...
  (let* ((iter (make-iter form file))
         (clause-type (cst:raw (consume-symbol iter))))
    (cond ((string-equal clause-type "IMPORT")
           (loop :while (next-value-p iter)
                 :do (parse-import-statement toplevel-package (consume-value iter) file)))
          ((string-equal clause-type "IMPORT-FROM")
           (parse-import-from toplevel-package iter))
          ((string-equal clause-type "EXPORT")
...
If the outermost form matches (eval-when (compile-toplevel) ..), evaluate the enclosed forms."
  (loop :for form :in (cst:raw (cst:rest form))
        :when (and (eql (car form) 'eval-when)
                   (some (lambda (k)
                           (string-equal k 'compile-toplevel))
                         (cadr form)))
