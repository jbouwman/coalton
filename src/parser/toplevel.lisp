...
(defun parse-import-statement (package cursor)
  (typecase (cursor:peek cursor)
    (list (let ((name (cursor:next-symbol cursor :missing "missing package name")))
            (cursor:next-symbol cursor :require "AS"
                                       :missing "missing AS")
            (let ((nick (cursor:next-symbol cursor :missing "missing package nickname")))
    ...
    (symbol (pushnew (symbol-name (cst:raw (cursor:cursor-form cursor)))
...
(defun parse-package-clause (package cursor)
  ...
  (let ((parser (package-clause-parser (cursor:next-symbol cursor))))
    (when (null parser)
      (let ((form (cursor:cursor-form cursor)))
...
(defun parse-symbol (cursor &key message missing require)
  (when (cursor:empty-p cursor)
    (cursor:syntax-error cursor
                         (or missing "expected symbol")))
  (cursor:next-if cursor
                  (lambda (value)
                    (when (not (symbolp value))
                      (cursor:syntax-error cursor
                                           (or message "expected symbol")))
                    (when (and require (not (string-equal require value)))
                      (cursor:syntax-error cursor
                                           (or message
                                               (format t "expected ~A" require)))))))

(defun parse-package (form)
  "Parse a coalton package declaration."
  (declare (type cst:cst form)
           (values toplevel-package))
  (let ((cursor (cursor:make-cursor form)))
    (cursor:next-symbol cursor
                 :require "PACKAGE"
                 :message "package declarations must start with `package`"
                 :missing "missing `package`")
    (let* ((package-name (cursor:next-symbol cursor
                                             :missing "missing package name"
                                             :not-symbol "package name must be a symbol"))
           (package-doc (when (not (cursor:empty-p cursor))
                          (cursor:next-if cursor #'stringp)))
...
(defun read-toplevel-package (stream file)
  ...
  (with-parser-package
    (handler-bind ((cursor:syntax-error (lambda (c)
                                          (cursor:make-parse-error file c))))
      (multiple-value-bind (form presentp)
          (maybe-read-form stream *coalton-eclector-client*)
        (unless presentp
...
(defun lisp-package (package)
  "Create a Lisp package by evlauating the defpackage representation of a PACKAGE form."
