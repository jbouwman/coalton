...
(defun parse-import (package cursor)
  ...
  (cursor:each-value cursor (alexandria:curry 'parse-import-statement package)))
         ...
         (package
           (make-toplevel-package :name (symbol-name package-name)
                                  ...
                                  :location (form-location (cursor:cursor-source cursor)
                                                           (cursor:cursor-value cursor)))))
    (cursor:each-value cursor (alexandria:curry #'parse-package-clause package))
    package))
  ...
  (let (unparsed-name
    ...
    (unless (cst:atom unparsed-name)
      (parse-error "Malformed constructor"
                   (note source unparsed-name "expected symbol")
                   (secondary-note source (cst:second enclosing-form) "in this type definition")))
...
(defun read-values (cursor accept &optional until)
  (let ((values nil))
    (loop :until (cursor:empty-p cursor)
          :do (when (and until (funcall until (cursor:peek cursor)))
                (return))
              (let ((value (cursor:next cursor :unwrap nil)))
                (push (funcall accept value) values)))
    (nreverse values)))

(defun parse-fundep (form source)
  (declare (type cst:cst form)
           (values fundep))
  (labels ((arrow-p (value)
             (eq value 'coalton:->))
           (parse-var (cst)
    ...
    (let* ((c (cursor:make-cursor form source "Malformed functional dependency"))
           (left (read-values c #'parse-var #'arrow-p)))
      (unless left
        (cursor:error c ':after-last "expected one or more type variables"))
      (cursor:next c)                   ; drop the arrow
      (let ((right (read-values c #'parse-var)))
        (unless right
          (cursor:error c ':after-last "expected one or more type variables"))
