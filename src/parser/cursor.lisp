...
(defun cursor-error (cursor message note &key end)
...
(defun end-location (cursor)
  (let ((span (cst:source (cursor-value cursor))))
    (source:make-location (cursor-source cursor)
                          (cons (1- (cdr span))
                                (1- (cdr span))))))

(defun end-error (cursor message note)
  (source:source-error message (source:make-note (end-location cursor) note)))
