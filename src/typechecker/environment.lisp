...
(defun add-early-types (env)
  "Add early types to a type environment"
  (env:set* env :type
...
(defun add-early-constructors (env)
  "Create add early constructors to an environment"
...
(defun replace-or-append (list element predicate)
  (let ((found-item (find-if predicate list)))
    (if found-item
        (substitute element found-item list)
        (cons element list))))
...
(defun add-specialization (env entry)
  (let* ((from (specialization-entry-from entry))
    ...
    (env:set
     ...
     from
     (replace-or-append
      (env:get env :specialization from)
      entry
      (lambda (elem)
...
(defun insert-fundep-entry% (env class fundep entry)
  (let ((result (lookup-fundep-environment env class)))
    (env:set env :fundep
             class
             (acons-list fundep entry result))))
