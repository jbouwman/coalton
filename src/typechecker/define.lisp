...
(defun error-ambiguous-pred (pred)
  ...
  (tc-error "Ambiguous predicate"
            (make-note (tc:ty-predicate-location pred)
                       "Ambiguous predicate ~S" pred)))
...
(defun standard-expression-type-mismatch-error (node subs expected-type ty)
  ...
  (tc-error "Type mismatch"
            (make-note (parser:node-location node)
  ...
  (check-duplicates
   ...
   (lambda (first second)
     (tc-error "Duplicate definition"
               (make-note (parser:node-location (parser:toplevel-define-name first))
                          "first definition here")
               (make-note (parser:node-location (parser:toplevel-define-name second))
  ...
  (check-duplicates
   ...
   (lambda (first second)
     (tc-error "Duplicate declaration"
               (make-note (parser:identifier-src-location (parser:toplevel-declare-name first))
                          "first declaration here")
               (make-note (parser:identifier-src-location (parser:toplevel-declare-name second))
  ...
  (loop :with def-table
        ...
        :unless (gethash name def-table)
          :do (tc-error "Orphan declaration"
                        (make-note (parser:identifier-src-location (parser:toplevel-declare-name declare))
    ...
    (check-duplicates
     ...
     (lambda (first second)
       (tc-error "Duplicate parameters name"
                 (make-note (parser:node-location first)
                            "first parameter here")
                 (make-note (parser:node-location second)
          ...
          (handler-case
            ...
            (tc:coalton-internal-type-error ()
              (tc-error "Return type mismatch"
                        ...
                        (make-note (parser:node-location
    ...
    (check-duplicates
     ...
     (lambda (first second)
       (tc-error "Duplicate definition in let"
                 (make-note (parser:node-let-binding-location first)
                            "first definition here")
                 (make-note (parser:node-let-binding-location second)
                            "second definition here"))))
      ...
      (multiple-value-bind (expr-ty preds accessors expr-node subs)
        ...
        (handler-case
          ...
          (tc:coalton-internal-type-error ()
            (tc-error "Type mismatch"
                      (make-note (parser:node-location node)
                                 "Declared type '~S' does not match inferred type '~S'"
    ...
    (let* ((preds nil)
      ...
      (handler-case
        ...
        (tc:coalton-internal-type-error ()
          (tc-error "Type mismatch"
                    (make-note (parser:node-location node)
                               "Expected type '~S' but 'or' evaluates to '~S'"
                               (tc:apply-substitution subs expected-type)
                               tc:*boolean-type*))))))
    ...
    (let* ((preds nil)
      ...
      (handler-case
        ...
        (tc:coalton-internal-type-error ()
          (tc-error "Type mismatch"
                    (make-note (parser:node-location node)
                               "Expected type '~S' but 'and' evaluates to '~S'"
                               (tc:apply-substitution subs expected-type)
                               tc:*boolean-type*))))))
      ...
      (multiple-value-bind (expr-ty preds accessors expr-node subs)
        ...
        (multiple-value-bind (ty_ pattern subs)
          ...
          (handler-case
            ...
            (tc:coalton-internal-type-error ()
              (tc-error "Type mismatch"
                        (make-note (parser:node-do-bind-location node)
                                   "Expected type '~S' but got '~S'"
                                   (tc:apply-substitution subs expected-type)
                                   (tc:apply-substitution subs expr-ty)))))))))
      ...
      (multiple-value-bind (ty preds_ accessors_ last-node subs)
        ...
        (handler-case
          ...
          (tc:coalton-internal-type-error ()
            (tc-error "Type mismatch"
                      (make-note (parser:node-location node)
                                 "Expected type '~S' but do expression has type '~S'"
                                 (tc:apply-substitution subs expected-type)
                                 (tc:apply-substitution subs ty)))))))))
  ...
  (:method ((pat parser:pattern-literal) expected-type subs env)
    ...
    (let ((ty (etypecase (parser:pattern-literal-value pat)
      ...
      (handler-case
        ...
        (tc:coalton-internal-type-error ()
          (tc-error "Type mismatch"
                    (make-note (parser:pattern-location pat)
                               "Expected type '~S' but pattern literal has type '~S'"
                               (tc:apply-substitution subs expected-type)
                               (tc:apply-substitution subs ty)))))))
      ...
      (check-duplicates (parser:pattern-variables pat)
                        ...
                        (lambda (first second)
                          (tc-error "Duplicate pattern variable"
                                    (make-note (parser:pattern-location first)
                                               "first definition here")
                                    (make-note (parser:pattern-location second)
                                               "second definition here"))))
      ...
      (let ((arity
        ...
        (unless (= arity num-args)
          (tc-error "Argument mismatch"
                    (make-note (parser:pattern-location pat)
                               "Constructor ~A takes ~D arguments but is given ~D"
        ...
        (let* ((ctor-ty (tc:qualified-ty-type ;; NOTE: Constructors cannot have predicates
          ...
          (handler-case
            ...
            (tc:coalton-internal-type-error ()
              (tc-error "Type mismatch"
                        (make-note (parser:pattern-location pat)
                                   "Expected type '~S' but pattern has type '~S'"
                                   (tc:apply-substitution subs expected-type)
    ...
    (loop :for binding :in bindings
          ...
          :if (gethash name def-table)
            :do (tc-error "Duplicate binding in let"
                          (make-note (parser:node-location (parser:node-let-binding-name binding))
                                     "second definition here")
                          (make-note (parser:node-location (parser:node-let-binding-name (gethash name def-table)))
                                     "first definition here"))
          :else
    ...
    (loop :for declare :in declares
          ...
          :if (gethash name dec-table)
            :do (tc-error "Duplicate declaration in let"
                          (make-note (parser:node-location (parser:node-let-declare-name declare))
                                     "second declaration here")
                          (make-note (parser:node-location (parser:node-let-declare-name
      ...
      (multiple-value-bind (accessors subs_)
        ...
        (when accessors
          (tc-error "Ambiguous accessor"
                    (make-note (accessor-location (first accessors))
...
(defun check-for-invalid-recursive-scc (bindings env)
  ...
  (when (and (some (alexandria:compose #'not #'parser:binding-function-p) bindings)
    ...
    (let ((first-fn (find-if #'parser:binding-function-p bindings)))
      ...
      (error 'tc-error
             ...
             :notes (cons (make-note (parser:node-location (parser:binding-name first-fn))
                                     "function can not be defined recursively with variables")
                          (loop :for binding :in (remove first-fn bindings :test #'eq)
                                :collect (make-secondary-note
                                          (parser:node-location (parser:binding-name binding))
                                          "with definition")))))
  ...
  (when (parser:binding-toplevel-p (first bindings))
    (error 'tc-error
           :message "Invalid recursive bindings"
           :notes (cons (make-note (parser:node-location (parser:binding-name (first bindings)))
  ...
  (let* ((param-tys (loop :with args := (tc:function-type-arguments expected-type)
                       ...
                       (tc:coalton-internal-type-error ()
                         (tc-error "Return type mismatch"
                                   (make-note (car (first *returns*))
                                              "First return is of type '~S'"
                                              (tc:apply-substitution subs (cdr (first *returns*))))
                                   (make-note (parser:node-location (parser:binding-last-node binding))
                                              "Second return is of type '~S'"
    ...
    (let ((ty (tc:make-function-type* param-tys ret-ty)))
      (handler-case
        ...
        (tc:coalton-internal-type-error ()
          (tc-error "Type mismatch"
                    (make-note (parser:binding-location binding)
