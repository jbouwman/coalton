     ...
     (tc-error (parser:node-source (parser:toplevel-define-name first))
               ...
               (list
                (se:make-source-error-note
                 :type :primary
                 :span (source-location-span (parser:node-source (parser:toplevel-define-name second)))
   ...
   (lambda (first second)
     (tc-error (parser:identifier-src-source (parser:toplevel-declare-name first))
               ...
               (list
                (se:make-source-error-note
                 :type :primary
                 :span (source-location-span (parser:identifier-src-source (parser:toplevel-declare-name second)))
      ...
      (let (;; Attach explicit types to any explicit bindings
        ...
        (loop :for define :in defines
              ...
              :do (setf env (tc:set-name env name (tc:make-name-entry
                                                   ...
                                                   :source (parser:binding-source define))))
     ...
     (lambda (first second)
       (tc-error (parser:node-source first)
                 ...
                 (list (se:make-source-error-note
                        :type :primary
                        :span (source-location-span (parser:node-source second))
    ...
    (check-duplicates
     ...
     (lambda (first second)
       (tc-error (parser:node-let-binding-source first)
                 ...
                 (list (se:make-source-error-note
                        :type :primary
                        :span (source-location-span (parser:node-let-binding-source second))
                        :message "second definition here")))))
                        ...
                        (lambda (first second)
                          (tc-error (parser:pattern-source first)
                                    ...
                                    (list
                                     (se:make-source-error-note
                                      :type :primary
                                      :span (source-location-span (parser:pattern-source second))
                                      :message "second definition here")))))
      ...
      (tc-error (parser:node-source (parser:binding-name first-fn))
                ...
                (loop :for binding :in (remove first-fn bindings :test #'eq)
                      :collect (se:make-source-error-note
                                :type :secondary
                                :span (source-location-span (parser:node-source (parser:binding-name binding)))
