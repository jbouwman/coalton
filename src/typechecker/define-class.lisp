  ...
  (check-duplicates
   ...
   (lambda (first second)
     (tc:tc-error "Duplicate class definition"
                  (source:make-note (parser:toplevel-define-class-head-location first)
                                    "first definition here")
                  (source:make-note (parser:toplevel-define-class-head-location second)
  ...
  (check-duplicates
   ...
   (lambda (first second)
     (tc:tc-error "Duplicate method definition"
                  (source:make-note (parser:method-definition-location first)
                                    "first definition here")
                  (source:make-note (parser:method-definition-location second)
                                    "second definition here"))))
    ...
    (check-duplicates
     ...
     (lambda (first second)
       (tc:tc-error "Duplicate class variable"
                    (source:make-note (parser:keyword-src-location first) ; TODO /-src//
                                      "first usage here")
                    (source:make-note (parser:keyword-src-location second) ; TODO /-src//
                                      "second usage here")))))
     ...
     (loop :for scc :in classes-by-scc
           ...
           :when (intersection superclass-names scc-names :test #'eq)
             :do (error 'tc:tc-error
                        ...
                        :notes (cons (source:make-note (parser:toplevel-define-class-head-location
                                                        (first scc))
                                                       "in class defined here")
                                     (loop :for class :in (rest scc)
                                           :collect (source:make-note
                                                     (parser:toplevel-define-class-head-location
                                                      class)
                                                     "in class defined here"))))
               ...
               (check-duplicates
                ...
                (lambda (first second)
                  (tc:tc-error "Duplicate variable in function dependency"
                                (source:make-note (parser:keyword-src-location first)
                                                  "first usage here")
                                (source:make-note (parser:keyword-src-location second)
                                                  "second usage here"))))))
      (loop :for fundep :in (parser:toplevel-define-class-fundeps class)
