...
;; Enironment and print utilities

(defun symbols-for-package (env ns package &optional (f #'identity))
  "Return the entries in namespace NS of environment ENV matching PACKAGE."
  (let ((result nil))
    (env:do-env (sym entry env ns)
      (when (eql package (symbol-package sym))
        (push (funcall f entry) result)))
    result))

(defun symbols-by-package (env ns &optional (f #'identity))
  "Group the contents of namespace NS in environment ENV by package."
  (let ((table (make-hash-table)))
    (env:do-env (sym entry env ns)
      (push (cons sym (funcall f entry))
            (gethash (symbol-package sym) table)))
    table))
...
(defun print-one-package (stream env entry-printer ns entry-resolver package)
  (let ((p (find-package package)))
    ...
    (print-package stream entry-printer p
                   (symbols-for-package p env ns entry-resolver))))
...
(defun print-all-packages (stream env entry-printer ns entry-resolver)
  (maphash (lambda (package entries)
           ...
           (symbols-by-package env ns entry-resolver)))
