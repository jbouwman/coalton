...
(defmethod name ((self source-file))
  (or (original-name self)
      (input-name self)))

(defmethod stream ((self source-file))
...
(defmethod stream ((self source-string))
  (make-string-input-stream (source-string self)))

(defmethod name ((self source-string))
...
(defun condition-stream (condition)
  (stream (location-source (location (first (notes condition))))))
