...
;;; char-position-stream
;;;
;;; The char-position-stream class wraps a character input stream, and
;;; reports stream position via #'file-position as number of
;;; characters read, rather than octets read, so that the source span
;;; offsets reported by concrete-syntax-tree are consistent across
;;; platforms and file and vector backed streams.
...
(defun make-note (location message &key end)
  (make-instance 'note
                 :location location
                 :message message
                 :end end))
...
(defun print-single-line-note (stream printer-state note)
  (multiple-value-bind (start-line start-column end-line end-column)
    ...
    (with-slots (note-max-depth note-stack) printer-state
      (write-nchar #\Space
                   (+ start-column
                      (if (note-end-p note) -1 0) ; TODO fixme
                      (- note-max-depth (length note-stack)))
                   stream)
      (format stream "~v{~C~:*~} ~A~%"
...
(defun report-source-condition (condition stream)
  (with-open-stream (source-stream (condition-stream condition))
    (let ((state (make-printer-state source-stream condition)))
      ...
      (with-slots (help context last-line) state ; TODO !w-s
